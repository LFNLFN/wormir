<template>
  <div class="app-container">
    <div class="mainContent">
      <div class="filter-container theTopMsgBorder no-border-bottom" style="border-width:1px; border-top-width:0px;" :style="{paddingBottom: 0, width: rowLength+'px'}">
        <el-row>
          <el-col :span="3" style="border-top:1px solid #d5d5d5;">
        ...
        <div class="twoTable-wrap" style="padding-right: 20px">
          ...
          <el-table :key=2 :data="totalTableData" border fit
            ...
            <el-table-column align="left" width="130" label="Description" prop="goodsChineseName">
              <template slot-scope="scope">
                <span v-if="scope.$index==totalTableData.length-1">{{ currentRow.refundSituation[currentRow.isAlreadyRefund][currentRow.refundReason].refundReasonText }}</span>
            ...
            <el-table-column width="140" align="right" label="Unit Purchase Price">
              <template slot-scope="scope">
                <span v-if="scope.$index==totalTableData.length-2" class="text-total">{{ 'Received 30% Deposit：' }}</span>
                <span v-else-if="scope.$index==totalTableData.length-1" class="text-total">{{ currentRow.refundSituation[currentRow.isAlreadyRefund][currentRow.refundReason].refundAmountTitle }}</span>
            ...
            <el-table-column align="center" width="130" label="30% Deposit">
              <template slot-scope="scope">
                ...
                <span v-else-if="scope.$index==totalTableData.length-1" class="text-total">￥ {{ deposit30Amount.toFixed(2) }}</span>
