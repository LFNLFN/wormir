<template>
  <div class="app-container">
    <h2 :style="{textAlign: 'center', width: rowLength+'px'}">已收货</h2>
          ...
          <div class="filter-container" style="padding-bottom: 0">
            <el-table key='replenishment' :data="order.replenishmentList" border fit
              ...
              <el-table-column width="140" align="center" :label="$t('order.orderAmount')">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-1&&currentOrder" class="text-total">￥ {{ currentOrder.total_channel_amount }}</span>
                </template>
              </el-table-column>
              <el-table-column align="center" width="130" :label="$t('order.thirtyPercentDeposit')">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-1&&currentOrder" class="text-total">￥ {{ currentOrder.total_earnest_amount }}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('order.residualPayment')" align="center" width="170">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-1&&currentOrder" class="text-total">￥ {{ currentOrder.total_surplus_amount }}</span>
                </template>
              </el-table-column>
            </el-table>
          </div>
  ...
  export default {
    ...
    methods: {
      async getList() {
        ...
        // 合计付款金额
        request({
          url: "/issue/issueOrderDetailTotalAmount.do",
          method: "post",
          data: { orderNo: this.currentRow.orderNo }
        }).then(res => {
          this.currentOrder = res.data.items;
        });
