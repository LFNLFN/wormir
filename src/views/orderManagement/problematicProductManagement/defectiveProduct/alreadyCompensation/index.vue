    ...
    <div class="theBorder" style="border-bottom-width: 1px">
      ...
      <template v-if="currentRow.compensation_type==11">
        ...
        <el-row>
          ...
          <el-col :offset="4" :span="14">
            <div class="text-muted">
              <span class="text-muted" v-if="currentRow.propertyOfSale==1">{{'补款已退回到您的帐户余额，可查看您账户余额的变化情况。' }}</span>
          <span class="text-muted" v-if="currentRow.propertyOfSale==2">{{'补款已原路退回，可查看您支付货款账户的金额变化情况。' }}</span>
            </div>
          </el-col>
        </el-row>
        <el-row>
