      ...
      <div :style="{paddingBottom: 0, marginTop: '-2px'}" class="twoTable-wrap">
        ...
        <div class="filter-container" style="padding-bottom: 0">
          ...
          >
            ...
            <el-table-column align="center" width="200" label="30% Deposit">
              <template slot-scope="scope">
          ...
          <el-table
            v-if="currentRow.refundTypeFront==1 || currentRow.refundTypeFront==3"
            key="productList1"
            :data="[{},{}]"
            border=""
            fit
            ...
            <el-table-column align="center" label="">
              <template slot-scope="scope">
                <div v-if="scope.$index==1">
                  <span v-if="currentRow.orderStatusFront==1">No longer produce，brand owner cancel the reservation.</span>
                  <span v-if="currentRow.orderStatusFront==2">Retailer has waited too long for stock and cancelled the order.</span>
                  <span v-if="currentRow.orderStatusFront==4">Retailer failed to pay residual payment, the order is terminated.</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column align="right" label="">
              <template slot-scope="scope">
                <div v-if="scope.$index==0">Received 30% Deposit：</div>
                <div v-if="scope.$index==1">
                  <span v-if="currentRow.orderStatusFront==4">Pending Deposit Compensation：</span>
                  <span v-else>Pending Refund of 30% Deposit：</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column align="center" width="200" label="">
              <template slot-scope="scope">
                <span v-if="currentRow.orderStatusFront==4">{{ (currentRow.total_earnest_amount*currentRow.compensationRate/100).toFixed(2) }}</span>
                <span v-else>{{ currentRow.total_earnest_amount }}</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
