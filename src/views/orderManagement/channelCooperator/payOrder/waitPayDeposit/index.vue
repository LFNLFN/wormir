    ...
    <div class="mainContent">
      ...
      <div style="display: flex">
        ...
        <div class="twoTable-wrap" style="padding-right: 20px">
          ...
          <!--补货表格-->
          <el-table v-if="hasReplenishment" key='replenishment' :data="order.replenishmentList" ref="replenishmentTable"
                    border fit size="mini"
          ...
          <el-table :key='2' :data="totalTableData" ref="totalTable"
            ...
            <el-table-column align="left" width="90" label="Retail Price">
              <template slot-scope="scope">
                ...
                <span v-else-if="scope.$index==totalTableData.length-1" class="text-muted"> 支付30%订金时间: {{ new Date() | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
            ...
            <el-table-column width="140" align="right" label="Unit Purchase Price">
              <template slot-scope="scope">
                <span class="text-total">{{ 'unpaid 30% deposit： ' }}</span>
              </template>
            </el-table-column>
            <el-table-column width="140" align="center" label="Total Purchase Price">
              <template slot-scope="scope">
            ...
            <el-table-column align="center" width="130" label="30% Deposit">
              <template slot-scope="scope">
                <span class="text-total">￥ {{ deposit30Amount.toFixed(2) }}</span>
              </template>
            </el-table-column>
          </el-table>
...
//        this.waitStockVisible = true
      ...
      // 弹层里面橙色总价
      payment_amount() {
        this.$emit('payment_amountChange', this.deposit30Amount)
      }
    },
    created() {
      this.getList()
