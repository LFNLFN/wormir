<template>
  <div class="app-container">
    <h2 :style="{paddingBottom: 0, width: rowLength+'px'}" class="text-center">待发货</h2>
      ...
      <div style="display: flex">
        <div class="twoTable-wrap" style="padding-right: 20px">
          ...
          <div class="filter-container" style="padding-bottom: 0">
            <el-table key='replenishment' :data="order.replenishmentList" border fit
              ...
              <el-table-column align="center" width="85" :label="$t('product.productNo')" prop="goodsNo">
                <template slot-scope="scope">
                  <div class="text-muted" v-if="scope.$index===order.replenishmentList.length-1">支付70%余款时间: {{ orderDetail.paySurplusTime }}
                  </div>
              ...
              <el-table-column align="center" width="130" :label="$t('order.thirtyPercentDeposit')">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-2" class="text-total">{{symbol}} {{ currentRow.total_earnest_amount }}</span>
                  <span v-else-if="scope.$index===order.replenishmentList.length-1" class="text-total">{{symbol}} {{ currentRow.total_channel_amount }}</span>
                </template>
              </el-table-column>
              <el-table-column :label="$t('order.residualPayment')" align="center" width="170">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-2" class="text-total">{{symbol}} {{ currentRow.total_surplus_amount }}</span>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <!--<div style="display: flex;justify-content: center;margin-top: 10px">-->
            <!--<el-button v-if="urgeBtnVisible" class="filter-item" style="width: 100px" type="primary" @click="makeUrge">催促发货</el-button>-->
            <!--<el-button v-if="cancelOrderBtnVisible" class="filter-item" style="width: 100px" type="primary" @click="cancelOrderProcess">取消订货</el-button>-->
          <!--</div>-->
        </div>
    ...
    <h2 :style="{paddingBottom: 0, width: rowLength+'px'}" class="text-center">转外币付余款</h2>
    <div class="mainContent">
      <div class="filter-container theBorder no-border-bottom" :style="{paddingBottom: 0, width: rowLength+'px'}">
        <el-row>
          <el-col :span="3">
      ...
      <div style="display: flex">
        <div class="twoTable-wrap" style="padding-right: 20px">
          ...
          <div class="filter-container" style="padding-bottom: 0">
            <div :style="{paddingBottom: 0, width: rowLength+'px'}">
              ...
              >
                ...
                <el-table-column label="70% Residual Payment" align="center">
                  <template slot-scope="scope">
                    <span>{{scope.row.symbol}} {{ scope.row.surplusAmount }}</span>
                  </template>
                </el-table-column>
          ...
          <div class="filter-container">
            <el-table key='replenishment' :data="order.replenishmentList1" border fit
                      size="mini"
                      style="width: 100%;border: 1px solid #d5d5d5;border-left-width: 2px;border-top:none;"
              ...
              <el-table-column align="right" :label="$t('price.orderUnitPrice')">
                <template slot-scope="scope">
                  <span class="text-total">{{'Unpaid 70％ Residual：'}}</span>
                </template>
              </el-table-column>
              ...
              <el-table-column label="70% Residual Payment" align="center">
                <template slot-scope="scope">
                  <span class="text-total">{{symbol}} {{ currentRow.total_surplus_amount }}</span>
                </template>
              </el-table-column>
            </el-table>
          </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        urgeBtnVisible: false,
        urgeVisible: false,
        cancelOrderBtnVisible: false,
        list: [],
    ...
    methods: {
      ...
      arraySpanMethod_replenishment1({ row, column, rowIndex, columnIndex }) {
        if (rowIndex === this.order.replenishmentList1.length - 1) {
          if (columnIndex === 7) {
            return {
              rowspan: 1,
              colspan: 10
      ...
      handleConfirm() {
        request({
          ...
          data: {
            ...
            statusChangeTo: -130, // 取消订单，待退全款
          }
        }).then(res => {
          if (res.errorCode == 0) {
            this.cancelOrderVisible = false;
            const vm = this
            this.$alert('该次订货已被取消，可在『一般贸易商品』“取消待退全款”状态中查看详情。', '', {
              confirmButtonText: this.$t('table.confirm'),
              showClose: false,
              callback() {
                vm.$emit('orderStatusChange',-130)
              }
            })
          }
        });
