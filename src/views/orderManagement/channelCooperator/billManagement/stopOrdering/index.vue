        ...
        <div class="twoTable-wrap" style="padding-right: 20px">
          ...
          <div class="filter-container">
            <el-table key='replenishment' :data="order.replenishmentList" border fit
              ...
              <el-table-column align="center" width="85" :label="$t('product.productNo')" prop="goodsNo">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-1" class="text-muted"> 取消订货时间: {{ new Date() | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                  <span v-else-if="scope.$index===order.replenishmentList.length-2&&currentRow.payWay===2" class="text-danger">{{ '温馨提醒：由于收到订金后才备货，所以请及时自行转帐，并核准以上帐号信息以人民币种转帐，因个人错误转帐造成的损失自行承担。' }}</span>
