<template>
  <div class="app-container">
            ...
            <el-table key='replenishment' :data="order.replenishmentList" border fit
              ...
              <el-table-column align="center" width="85" :label="$t('product.productNo')" prop="goodsNo">
                <template slot-scope="scope">
                  ...
                  <div class="text-muted" v-else-if="scope.$index===order.replenishmentList.length-1">完成备货时间: {{ new
              ...
              <el-table-column align="center" width="140" :label="$t('price.startDiscount')">
                <template slot-scope="scope">
                  <div v-if="scope.$index===order.replenishmentList.length-1" class="text-total">{{ '70%余款' }} :
                  </div>
    ...
    <el-dialog :visible.sync="completePaymentVisible" width="30%" append-to-body :show-close="false">
      <div style="text-align: center">货单号 <span>{{currentRow.orderNo}}</span> 已完成余款支付，<br>当前帐户余额 ￥ <span>{{(accountResidual-deposit30).toFixed(2)}}</span><br>可在待备货状态项下『查看货单』。
      </div>
      <div slot="footer" class="dialog-footer" style="text-align: center">
