        ...
        <div class="twoTable-wrap" style="padding-right: 20px">
          ...
          <div class="filter-container" style="padding-bottom: 0">
            <el-table key='replenishment' :data="order.replenishmentList" border fit
              ...
              <el-table-column align="center" width="85" :label="$t('product.productNo')" prop="goodsNo">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-3"
                        class="text-danger">{{ '温馨提醒：请于XXXX年XX月XX日24点前完成余款支付，否则取消备货，订金不予退回。' }}</span>
                  <div class="text-muted" v-else-if="scope.$index===order.replenishmentList.length-2">支付30%订金时间: {{ new
                    Date(completeStockTime) | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}
                  </div>
                  <div class="text-muted" v-else-if="scope.$index===order.replenishmentList.length-1">取消备货时间: {{ new
                    Date(completeStockTime) | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}
                  </div>
              ...
              <el-table-column align="center" width="140" :label="$t('price.startDiscount')">
                <template slot-scope="scope">
                  <div v-if="scope.$index===order.replenishmentList.length-2" class="text-total">{{ currentRow.channelClassify===2?
                    '70%余款+跨境税金' : '70%余款' }} :
                  </div>
              ...
              <el-table-column align="center" width="130" :label="$t('order.thirtyPercentDeposit')">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-3" class="text-total">{{ '合计: ' }}</span>
              ...
              <el-table-column :label="$t('order.residualPayment')" align="center" width="170">
                <template slot-scope="scope">
                  ...
                  <div v-else-if="scope.$index===order.replenishmentList.length-2" class="text-total">￥ {{ currentRow.channelClassify===2? (deposit70Amount+crossCountryAmount).toFixed(2) :
                    deposit70Amount.toFixed(2) }}
                  </div>
                  <div v-else-if="scope.$index===order.replenishmentList.length-1" class="text-danger">{{ '已取消备货' }}
                  </div>
              ...
              <el-table-column label="跨境税金" align="center" width="170" v-if="currentRow.channelClassify===2">
                <template slot-scope="scope">
                  <span
                    v-if="scope.$index===order.replenishmentList.length-3" class="text-total">￥ {{ crossCountryAmount.toFixed(2)}}</span>
          ...
          <!--<div style="display: flex;justify-content: flex-end;">-->
            <!--<el-button class="filter-item" type="primary" style="float: right;" @click="goPayDeposit()">去付余款-->
            <!--</el-button>-->
          <!--</div>-->
        </div>
  ...
  export default {
    ...
    computed: {
      deposit30Amount() {
        ...
        return '￥ ' + deposit30_amount.toFixed(2)
      },
      deposit70Amount() {
        let itemOrder_amount = 0
        this.order.orderSubs.forEach((subsItem, index) => {
