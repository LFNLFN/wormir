<template>
  <div class="app-container">
    <h2 :style="{paddingBottom: 0, width: rowLength+'px'}" class="text-center">缺货等待</h2>
    <div class="mainContent">
      ...
      <div style="display: flex">
        <div class="twoTable-wrap" style="padding-right: 20px">
          ...
          <div class="filter-container" style="padding-bottom: 0">
            <el-table key='replenishment' :data="order.replenishmentList" border fit
              ...
              <el-table-column align="center" width="85" :label="$t('product.productNo')" prop="goodsNo">
                <template slot-scope="scope">
                  <span v-if="scope.$index===order.replenishmentList.length-1" class="text-muted"> 支付30%订金时间: {{ orderDetail.shortageWaitingTime }}</span>
          ...
          <!--<div style="display: flex;justify-content: center;margin-top: 10px" v-if="cancelOrderAvailable">-->
            <!--<el-button class="filter-item" type="primary" @click="cancelOrderProcess">取消订货-->
            <!--</el-button>-->
          <!--</div>-->
    ...
    methods: {
      getList() {
        request({
          ...
          this.cancelOrderAvailable = this.orderDetail.cancelOrderAvailable
        });
      },
      ...
      handleConfirm() {
        this.$request({
          ...
          data: {
            ...
            statusChangeTo: -110, // 取消订货（备货时间过长，待退订金）
          }
        }).then(res => {
          if (res.errorCode == 0) {
            this.cancelOrderVisible = false;
            const vm = this
            this.$alert('该次订货已被取消，可在“取消待退订金”状态中查看详情。', '', {
              confirmButtonText: this.$t('table.confirm'),
              showClose: false,
              callback() {
                vm.$emit('orderStatusChange',-110)
                vm.$emit('closeDialog')
              }
            })
          }
